    ...
    override suspend fun loadLinks(
        ...
        for (a in asList) {
            ...
            if (href.startsWith("magnet:")) {
                ...
                callback.invoke(
                    ExtractorLink(source = name, name = linkName, url = href, referer = "", quality = 0, type = com.lagradost.cloudstream3.utils.ExtractorLinkType.VIDEO)
                )
            } else if (href.lowercase().endsWith(".torrent")) {
                var linkName = a.text().trim()
                if (linkName.isEmpty()) linkName = "Torrent"
                callback.invoke(
                    ExtractorLink(source = name, name = linkName, url = fixUrl(href), referer = "", quality = 0, type = com.lagradost.cloudstream3.utils.ExtractorLinkType.VIDEO)
                )
            }
        }
        return true
