    ...
    override fun onBindingCreated(binding: QuickSearchBinding) {
        ...
        binding.searchFilter.setOnClickListener { searchView ->
            searchView?.context?.let { ctx ->
                ...
                val dialogView = android.view.LayoutInflater.from(ctx).inflate(com.lagradost.cloudstream3.R.layout.dialog_quick_search_filter, null)
                val builder = com.google.android.material.dialog.MaterialAlertDialogBuilder(ctx, com.lagradost.cloudstream3.R.style.AlertDialogCustom)
                    .setView(dialogView)
                val dialog = builder.create()
                dialog.show()
                
                val cancelBtt = dialogView.findViewById<com.google.android.material.button.MaterialButton>(com.lagradost.cloudstream3.R.id.cancel_btn)
                val applyBtt = dialogView.findViewById<com.google.android.material.button.MaterialButton>(com.lagradost.cloudstream3.R.id.apply_btn)
                val listView = dialogView.findViewById<android.widget.ListView>(com.lagradost.cloudstream3.R.id.listview_providers)
                val checkboxAll = dialogView.findViewById<com.google.android.material.checkbox.MaterialCheckBox>(com.lagradost.cloudstream3.R.id.checkbox_all)
                
                val arrayAdapter = android.widget.ArrayAdapter<String>(ctx, com.lagradost.cloudstream3.R.layout.item_provider_checkbox)
                ...
                val names = currentValidApis.map {
                    if (isMultiLang) "${com.lagradost.cloudstream3.utils.SubtitleHelper.getFlagFromIso(it.lang)?.plus(" ") ?: ""}${it.name}" else it.name
                }
