...
import com.unan.nexxxup.databinding.FragmentHomeBinding
import com.unan.nexxxup.databinding.HomeEpisodesExpandedBinding
import com.unan.nexxxup.databinding.HomeSelectMainpageBinding
import com.unan.nexxxup.databinding.TvtypesChipsBinding
...
import com.unan.nexxxup.ui.search.SEARCH_ACTION_PLAY_FILE
import com.unan.nexxxup.ui.search.SearchAdapter
import com.unan.nexxxup.ui.search.SearchHelper.handleSearchClickCallback
...
import com.unan.nexxxup.utils.AppContextUtils.isRecyclerScrollable
...
import com.unan.nexxxup.utils.BackPressedCallbackHelper
...
import com.unan.nexxxup.utils.EmptyEvent
import com.unan.nexxxup.utils.SubtitleHelper.getFlagFromIso
...
import com.unan.nexxxup.utils.UIHelper.popupMenuNoIconsAndNoStringRes
...
class HomeFragment : BaseFragment<FragmentHomeBinding>(
    ...
    companion object {
        ...
        private fun getPairList(
            ...
            return listOf(
                Pair(all, listOf(TvType.Others)),
                Pair(asian, listOf(TvType.AsianDrama)),
                Pair(western, listOf(TvType.NSFW)),
                Pair(webcam, listOf(TvType.Live)),
            )
        }

        private fun getPairList(header: TvtypesChipsBinding) = getPairList(
            header.homeSelectAll,
            header.homeSelectAsian,
            header.homeSelectWestern,
            header.homeSelectWebcam
        )
        ...
        fun Context.selectHomepage(selectedApiName: String?, callback: (String) -> Unit) {
            ...
            builder.let { dialog ->
                ...
                bindChips(
                    ...
                    preSelectedTypes,
                    validAPIs.flatMap { it.getCustomAdultTypes() }.distinct()
                ) { list ->
    ...
    private var gridAdapter: com.unan.nexxxup.ui.search.SearchAdapter? = null
    private var concatAdapter: androidx.recyclerview.widget.ConcatAdapter? = null
    private var currentCategoryIndex = 0

    private fun updateGridCategory() {
        val data = homeViewModel.page.value
        if (data is Resource.Success) {
            val d = data.value
            val currentCategory = d.values.elementAtOrNull(currentCategoryIndex)
            if (currentCategory != null) {
                // Update grid layout manager spans because horizontal images exist
                val layoutManager = binding?.homeMasterRecycler?.layoutManager as? androidx.recyclerview.widget.GridLayoutManager
                if (layoutManager != null) {
                    layoutManager.spanCount = context?.getSpanCount(currentCategory.list.isHorizontalImages) ?: 3
                }
                ...
                gridAdapter = com.unan.nexxxup.ui.search.SearchAdapter(
                ...
                concatAdapter?.addAdapter(gridAdapter!!)
                
                gridAdapter?.submitList(currentCategory.list.list)
            } else {
    ...
    override fun onBindingCreated(binding: FragmentHomeBinding) {
        ...
        binding.apply {
            ...
            homeMasterRecycler.setRecycledViewPool(com.unan.nexxxup.ui.search.SearchAdapter.sharedPool)
            homeMasterRecycler.adapter = concatAdapter
            
            val spanCount = context?.getSpanCount(false) ?: 3
            val layoutManager = androidx.recyclerview.widget.GridLayoutManager(context, spanCount)
            layoutManager.spanSizeLookup = object : androidx.recyclerview.widget.GridLayoutManager.SpanSizeLookup() {
                override fun getSpanSize(position: Int): Int {
                    return if (position == 0) layoutManager.spanCount else 1
                }
            }
            homeMasterRecycler.layoutManager = layoutManager
            ...
            val isSingleProviderHome = try {
                findNavController().currentDestination?.id == R.id.navigation_single_provider_home
            } catch (e: Exception) { false }
            
            val homePreviewBackButton = view?.findViewById<ImageView>(R.id.home_preview_back_button)
            if (homePreviewBackButton != null) {
                homePreviewBackButton.isVisible = isSingleProviderHome
                homePreviewBackButton.setOnClickListener {
                    try {
            ...
            homeMasterRecycler.addOnScrollListener(object : RecyclerView.OnScrollListener() {
                ...
                override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                    ...
                    val currentAdapter = gridAdapter ?: return
                    val count = currentAdapter.itemCount
                    val currentHasNext = currentAdapter.hasNext
                    
                    if (!recyclerView.isRecyclerScrollable() && currentHasNext && expandCount != count) {
                        expandCount = count
                        val data = homeViewModel.page.value
                        if (data is Resource.Success) {
                            val name = data.value.keys.elementAtOrNull(currentCategoryIndex) ?: return
                            ioSafe {
                                homeViewModel.expandAndReturn(name)?.let { newExpand ->
                ...
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                    if (isLayout(PHONE)) {
                        ...
                        val backButtonScroll = this@HomeFragment.view?.findViewById<ImageView>(R.id.home_preview_back_button)

                        if (view == null) {
                            // The first view is not visible, so we can assume we have scrolled past it
                            scrollParent.isVisible = false
                            if (backButtonScroll?.isVisible == true && isSingleProviderHome) backButtonScroll.isVisible = false
                        } else {
                            ...
                            if (isSingleProviderHome) {
                                backButtonScroll?.isVisible = true
                                backButtonScroll?.translationY = rect[1].toFloat() - 60.toPx
                            }
                        }
                    }
        ...
        observe(homeViewModel.page) { data ->
            binding.apply {
                when (data) {
                    is Resource.Success -> {
                        val d = data.value
                        homeMasterAdapter?.submitList(emptyList()) // empty list for categories so only header renders
