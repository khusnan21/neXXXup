...
class ExtensionsFragment : BaseFragment<FragmentExtensionsBinding>(
        ...
        ioSafe {
            ...
            uris.amap { uri ->
                ...
                try {
                    ...
                    if (fileName.endsWith(".nex", ignoreCase = true)) {
                        failedList.add(fileName to "Gagal: Format .nex tidak didukung di sini. Silakan gunakan tombol khusus .nex.")
                        return@amap
                    }
                    ...
                    if (success) {
                        ...
                        try {
                            val cloudStreamFolder = android.os.Environment.getExternalStorageDirectory().absolutePath + "/Cloudstream3"
    ...
    private val localNexPluginPicker = registerForActivityResult(ActivityResultContracts.GetMultipleContents()) { uris ->
        if (uris.isNullOrEmpty()) return@registerForActivityResult
        val context = context ?: return@registerForActivityResult
        ioSafe {
            val successList = java.util.Collections.synchronizedList(mutableListOf<String>())
            ...
            uris.amap { uri ->
                ...
                try {
                    ...
                    if (fileName.endsWith(".cs3", ignoreCase = true)) {
                        failedList.add(fileName to "Gagal: Format .cs3 tidak didukung di sini. Silakan gunakan tombol khusus standard.")
                        return@amap
                    }
                    if (!fileName.endsWith(".nex", ignoreCase = true)) {
                        failedList.add(fileName to "Gagal: Format tidak didukung. Hanya mendukung file .nex.")
                        return@amap
                    }
                    ...
                    val success = com.unan.nexxxup.plugins.NexExtensionHelper.loadNexPlugin(context, destinationFile)
                    if (success) {
                        // Also attempt to copy to the legacy public directory for backwards compatibility, but ignore failures silently
                        try {
                            val cloudStreamFolder = android.os.Environment.getExternalStorageDirectory().absolutePath + "/Cloudstream3"
                        ...
                        failedList.add(fileName to "Gagal memuat plugin Nex: File tidak valid atau rusak.")
                    }
                } catch (e: Exception) {
                    com.unan.nexxxup.mvvm.logError(e)
                    failedList.add(fileName to "Error: ${e.message}")
    ...
    override fun onBindingCreated(binding: FragmentExtensionsBinding) {
        ...
        val addNexRepositoryClick = View.OnClickListener {
            val ctx = context ?: return@OnClickListener
            val binding = AddRepoInputBinding.inflate(LayoutInflater.from(ctx), null, false)
            binding.text1.text = "Tambah Repo NEX"
            binding.repoNameInput.setHint("Nama Repo NEX (Opsional)")
            binding.repoUrlInput.setHint("URL Repo NEX")
            ...
            binding.applyBtt.setOnClickListener secondListener@{
                ...
                ioSafe {
                    ...
                    if (url.isNullOrBlank()) {
                        ...
                        val newRepo = RepositoryData(repository.iconUrl, fixedName, url)
                        RepositoryManager.addNexRepository(newRepo)
                        extensionViewModel.loadStats()
                        extensionViewModel.loadRepositories()
        ...
        binding.apply {
            ...
            installLocalNexCard.setOnClickListener {
                localNexPluginPicker.launch("*/*")
            }
            addRepoCard.setOnClickListener(addRepositoryClick)
            addRepoNexCard.setOnClickListener(addNexRepositoryClick)
