...
import com.unan.nexxxup.databinding.FragmentHomeHeadBinding
import com.unan.nexxxup.databinding.FragmentHomeHeadTvBinding
...
import com.unan.nexxxup.ui.result.getId
...
import com.unan.nexxxup.utils.SingleSelectionHelper.showOptionSelectStringRes
import com.unan.nexxxup.utils.UIHelper.fixPaddingStatusbarMargin
import com.unan.nexxxup.utils.UIHelper.fixPaddingStatusbarView
...
class HomeParentItemAdapterPreview(
    ...
    val onCategoryClick: ((Int) -> Unit)? = null
) : ParentItemAdapter(
    ...
    override fun onCreateHeader(parent: ViewGroup): ViewHolderState<Bundle> {
        ...
        return HeaderViewHolder(binding, viewModel, fragment, onCategoryClick)
    }
    ...
    private class HeaderViewHolder(
        ...
        fun onSelect(item: LoadResponse, position: Int) {
            ...
            (binding as? FragmentHomeHeadBinding)?.apply {
                ...
                homePreviewBookmarkIcon.setImageDrawable(
                    ContextCompat.getDrawable(
                        homePreviewBookmarkIcon.context,
                        watchType.iconRes
                    )
                )

                homePreviewBookmarkHolder.setOnClickListener { fab ->
        ...
        init {
            val backIcon = itemView.findViewById<View>(R.id.home_back_icon)
            if (backIcon != null) {
                val isSingleProviderMode = try {
                    (fragment as? androidx.fragment.app.Fragment)?.findNavController()?.currentDestination?.id == R.id.navigation_single_provider_home
                } catch (e: Exception) {
                    false
                }
                
                backIcon.isVisible = isSingleProviderMode
                backIcon.setOnClickListener {
                    try {
                        (fragment as? androidx.fragment.app.Fragment)?.findNavController()?.popBackStack()
                    } catch (e: Exception) {
                        e.printStackTrace()
                    }
                }
            ...
            itemView.findViewById<View>(R.id.home_search_icon)?.setOnClickListener {
                viewModel.apiName.value?.let { apiName ->
                    com.unan.nexxxup.ui.quicksearch.QuickSearchFragment.pushSearch(activity, null, arrayOf(apiName))
                } ?: run {
                    activity?.navigate(R.id.navigation_quick_search)
                }
            }

            itemView.findViewById<View>(R.id.home_download_icon)?.setOnClickListener {
            ...
            (binding as? FragmentHomeHeadBinding)?.apply {
                // ... (removing the alternateHeadProfile logic and the empty scope comment if that matches)
            }
        ...
        fun onViewAttachedToWindow() {
            ...
            binding.root.findViewTreeLifecycleOwner()?.apply {
                ...
                observe(viewModel.page) { result ->
                    if (result is Resource.Success && binding is FragmentHomeHeadBinding) {
                        val chipGroup = binding.homeCategoryChipGroup
                        if (chipGroup != null) {
                            chipGroup.removeAllViews()
                            val categories = result.value.keys.toList()
                            val recyclerView = binding.root.parent as? androidx.recyclerview.widget.RecyclerView
                            if (categories.isNotEmpty()) {
                                binding.homeCategoryChipsScroll?.isVisible = true
                                categories.forEachIndexed { index, name ->
                                    val chip = com.google.android.material.chip.Chip(chipGroup.context).apply {
                                        ...
                                        isClickable = true
                                        setOnClickListener {
                                            onCategoryClick?.invoke(index)
                                        }
                                    }
                                    chipGroup.addView(chip)
                                }
                            } else {
                                binding.homeCategoryChipsScroll?.isGone = true
                            }
                        }
                    }
                }
