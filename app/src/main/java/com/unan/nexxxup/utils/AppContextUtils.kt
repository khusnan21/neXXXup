...
import com.unan.nexxxup.DubStatus
...
import com.unan.nexxxup.ui.WebviewFragment
import com.unan.nexxxup.ui.player.SubtitleData
import com.unan.nexxxup.ui.result.ResultFragment
import com.unan.nexxxup.ui.settings.Globals
import com.unan.nexxxup.ui.settings.extensions.PluginsFragment
import com.unan.nexxxup.ui.settings.extensions.RepositoryData
...
import com.unan.nexxxup.utils.FillerEpisodeCheck.toClassDir
import com.unan.nexxxup.utils.JsUnpacker.Companion.load
...
//        hashSet.add("en") // def is only en
    ...
    fun Context.filterProviderByPreferredMedia(hasHomePageIsRequired: Boolean = true): List<MainAPI> {
        ...
        // java.lang.ClassCastException: com.unan.nexxxup.TvType cannot be cast to com.unan.nexxxup.TvType
    ...
    fun MainAPI.getCustomAdultTypes(): Set<TvType> {
        val className = this.javaClass.name.lowercase()
        val nameLower = this.name.lowercase()
        val types = mutableSetOf<TvType>(TvType.Others) // representing "all"
        
        val isWebcam = className.contains("stripchat") || className.contains("chaturbate") || className.contains("cam4") ||
                className.contains("camsoda") || className.contains("camwh") ||
                nameLower.contains("stripchat") || nameLower.contains("chaturbate") || nameLower.contains("cam4") ||
                nameLower.contains("camsoda") || nameLower.contains("camwh") || nameLower.contains("livecam")
        
        val isAsian = className.contains("jav") || className.contains("asian") || className.contains("desi") ||
                className.contains("korean") || className.contains("hanime") || className.contains("hentai") ||
                className.contains("cxxx") || className.contains("wumaobi") || className.contains("xchina") ||
                nameLower.contains("jav") || nameLower.contains("asian") || nameLower.contains("desi") ||
                nameLower.contains("korean") || nameLower.contains("hanime") || nameLower.contains("hentai") ||
                nameLower.contains("cornhub") || nameLower.contains("netflav") || nameLower.contains("vlxx") ||
                nameLower.contains("xnxx") || nameLower.contains("ymaal") || nameLower.contains("ixiporn") ||
                nameLower.contains("asianpinay") || nameLower.contains("desixflix")
                
        if (isWebcam) {
            types.add(TvType.Live)
        } else if (isAsian) {
            types.add(TvType.AsianDrama)
        } else {
            types.add(TvType.NSFW)
        }
        return types
    }
