...
class JavGuru : MainAPI() {
    ...
    private var cachedActresses = listOf<SearchResponse>()
    ...
    override val mainPage = mainPageOf(
        mainUrl to "Home",
        "$mainUrl/jav-actress-list" to "Actresses",
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val isActressList = request.data.contains("jav-actress-list", ignoreCase = true)

        if (isActressList) {
            val itemsPerPage = 40
            if (page == 1 || cachedActresses.isEmpty()) {
                val url = "${request.data}/"
                Log.d("Nexxxup", "MainPage Actress URL: $url")
                val document = app.get(url, headers = mainHeaders).document
                val items = document.select(".actress-box a")
                cachedActresses = items.mapNotNull { it.toSearchResponse() }
            }

            val startIndex = (page - 1) * itemsPerPage
            if (startIndex >= cachedActresses.size) {
            ...
            val endIndex = minOf(startIndex + itemsPerPage, cachedActresses.size)
            val chunk = cachedActresses.subList(startIndex, endIndex)
            val hasNext = endIndex < cachedActresses.size
        ...
        Log.d("Nexxxup", "MainPage URL: $url")

        val document = app.get(url, headers = mainHeaders).document
        val items = document.select("div.inside-article, article, div.tabcontent li, .item-list li, .actress-box a")
    ...
    private fun Element.toSearchResponse(): SearchResponse? {
        val linkElement = this.selectFirst("div.imgg a, h2 a") ?: if (this.tagName() == "a") this else this.selectFirst("a")
        val href = fixUrlNull(linkElement?.attr("href")) ?: return null

        val imgElement = this.selectFirst("img")
        var title = imgElement?.attr("alt")?.trim()?.ifBlank { null }
            ?: this.selectFirst(".actrees-name, .actress-name")?.text()?.trim()
        ...
        val videoCount = this.selectFirst(".actress-details span:not(.actrees-name):not(.actress-name)")?.text()?.trim()
        val displayName = if (!videoCount.isNullOrBlank()) {
            "$title ($videoCount)"
        } else {
            title
        }
        ...
        val isActress = href.contains("/actress/")
        if (isActress) {
            return newTvSeriesSearchResponse(displayName, href, TvType.TvSeries) {
                this.posterUrl = posterUrl
                this.posterHeaders = mainHeaders
            }
        } else {
            return newMovieSearchResponse(displayName, href, TvType.NSFW) {
                this.posterUrl = posterUrl
                this.posterHeaders = mainHeaders
            }
        }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        if (url.contains("/actress/")) {
            ...
            fun parseDoc(doc: org.jsoup.nodes.Document) {
                val cards = doc.select("div.inside-article, article, div.tabcontent li, .item-list li")
                cards.forEach { card ->
                    val epurlElement = card.selectFirst("div.imgg a, h2 a") ?: if (card.tagName() == "a") card else card.selectFirst("a")
                    val epurl = epurlElement?.attr("href") ?: return@forEach
                    val epimgElement = card.selectFirst("img")
                    val epname = epimgElement?.attr("alt")?.trim()
                        ?: card.selectFirst("h2")?.text()?.trim()
                        ?: epurlElement?.text()?.trim()
                        ?: ""
                    val epposter = epimgElement?.attr("src") ?: epimgElement?.attr("data-src")
            ...
            if (maxPageLimit > 1) {
                val pages = (2..maxPageLimit).toList().amap { p ->
                    try {
                        val pageRes = app.get("${url.trimEnd('/')}/page/$p/", headers = mainHeaders)
            ...
            val docTitle = doc1.selectFirst("h1.tit1")?.text()?.trim()
                ?: doc1.selectFirst("h1")?.text()?.trim()
                ?: doc1.title().substringBefore("Archives").trim()
            
            val title = if (docTitle.isNotBlank()) docTitle else url.substringAfterLast("/actress/").replace("-", " ").replace("/", "").replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }
