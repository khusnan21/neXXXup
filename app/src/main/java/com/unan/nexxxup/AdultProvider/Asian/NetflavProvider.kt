...
class NetflavProvider : MainAPI() {
    ...
    private fun Element.toSearchResult(script: String): SearchResponse {
        ...
        val posterUrl = if (script.contains("\"code\":\"${code}\",\"preview_hp\":\"")) {
    ...
    private fun Element.toSearchResult2(script: String): SearchResponse {
        ...
        var code = removeSquareBracketsContent(title.split(" ").get(0))
        val href = fixUrl(this.selectFirst("a")?.attr("href") ?: "")
        val posterUrl = if (script.contains("\",\"code\":\"${code}\",")) {
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        val response = document.select("script").find { it.data().contains("eval(function(p,a,c,k,e,d)") }?.data()?.let { JsUnpacker(it).unpack() ?: "" } ?: ""
            ...
            if (link.isNotEmpty()) {
                callback.invoke(
                    newExtractorLink(
                        name,
                        "$name [$key]",
                        fixUrlLocal(link, getBaseUrl(data)),
    ...
    private fun fixUrlLocal(url: String, domain: String): String {
    ...
    private fun removeSquareBracketsContent(input: String): String {
