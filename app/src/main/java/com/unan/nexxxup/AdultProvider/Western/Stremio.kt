...
import com.unan.nexxxup.Actor
import com.unan.nexxxup.ActorData
import com.unan.nexxxup.Episode
...
import com.unan.nexxxup.LoadResponse.Companion.addImdbId
import com.unan.nexxxup.LoadResponse.Companion.addTMDbId
import com.unan.nexxxup.LoadResponse.Companion.addTrailer
import com.unan.nexxxup.MainAPI
import com.unan.nexxxup.MainPageData
...
import com.unan.nexxxup.ShowStatus
import com.unan.nexxxup.SubtitleFile
import com.unan.nexxxup.newSubtitleFile
import com.unan.nexxxup.TvType
import com.unan.nexxxup.addDate
import com.unan.nexxxup.amap
import com.unan.nexxxup.app
import com.unan.nexxxup.fixTitle
import com.unan.nexxxup.imdbUrlToIdNullable
import com.unan.nexxxup.newEpisode
...
import com.unan.nexxxup.newTvSeriesLoadResponse
...
class Stremio : MainAPI() {
    ...
    override var name = "Streamio"
    override var lang = "it"
    override val supportedTypes = setOf(TvType.Others, TvType.NSFW, TvType.Torrent, TvType.Movie, TvType.TvSeries)
    override val hasMainPage = true

    // Default configuration injected directly
    private val catalogUrl: String? = null
    private val streamAddons = listOf(
        StreamAddonConfig(
            id = 1L,
            name = "Torrentio",
    ...
    private suspend fun invokeCatalogStream(
        ...
        val encodedId = URLEncoder.encode(loadData.id.toString(), "UTF-8")
        val request = app.get(
    ...
    private suspend fun getTmdbMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val adultQuery = "&without_keywords=190370|13059|226161|195669|190370"
    ...
    private suspend fun searchTmdb(query: String, page: Int): SearchResponseList? {
        return app.get(
            "$TMDB_API/search/multi?api_key=$API_KEY&language=it&query=$query&page=$page&include_adult=false"
        ).parsedSafe<TmdbResults>()?.results?.mapNotNull { media ->
    ...
    private suspend fun loadFromTmdb(url: String): LoadResponse {
        ...
        val res = app.get(
            if (type == TvType.Movie) {
                "$TMDB_API/movie/${data.id}?api_key=$API_KEY&language=it&append_to_response=keywords,credits,external_ids,videos,recommendations"
            } else {
                "$TMDB_API/tv/${data.id}?api_key=$API_KEY&language=it&append_to_response=keywords,credits,external_ids,videos,recommendations"
            }
        ...
        return if (type == TvType.TvSeries) {
            ...
            res.seasons?.forEach { season ->
                val seasonUrl = "$TMDB_API/tv/${data.id}/season/${season.seasonNumber}?api_key=$API_KEY&language=it"
    ...
    data class TmdbMedia(
        ...
        fun toSearchResponse(provider: Stremio, type: String? = null): SearchResponse? {
...
data class Catalog(
    ...
    suspend fun search(query: String, catUrl: String, provider: Stremio): List<SearchResponse> {
    ...
    suspend fun toHomePageList(catUrl: String, provider: Stremio): HomePageList {
        val entries = mutableListOf<SearchResponse>()
        types.forEach { type ->
...
data class CatalogEntry(
    ...
    suspend fun toLoadResponse(provider: Stremio, imdbId: String?): LoadResponse {
...
data class CatalogVideo(
    ...
    fun toEpisode(provider: Stremio, type: String?, imdbId: String?): Episode {
...
data class StreamsResponse(val streams: List<StremioStream>)
...
object SubsExtractors {
    suspend fun invokeOpenSubs(
        ...
        app.get("https://opensubtitles-v3.strem.io/subtitles/$slug.json", timeout = 120L).parsedSafe<OsResult>()?.subtitles?.map { sub ->
            subtitleCallback.invoke(
                newSubtitleFile(
                    SubtitleHelper.fromTagToEnglishLanguageName(sub.lang ?: "") ?: sub.lang
                    ?: return@map,
