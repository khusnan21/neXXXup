...
class AllClassicPorn(val context: Context) : MainAPI() {
...
//        "${mainUrl}/categories/gay/"                to  "Gay",
    ...
    suspend fun getVideoUrl(context: Context, pageHtml: String): String? {
        return suspendCoroutine { continuation ->
            // Main thread'de çalıştır - WebView için gerekli
            CoroutineScope(Dispatchers.Main).launch {
                createWebViewAndExtractVideo(context, pageHtml) { result ->
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
       ...
       Log.d("kraptor_${this.name}", "WebView ile kt_player video URL'si çıkarılıyor...")
        ...
        val videoUrl = getVideoUrl(context, pageHtml)
